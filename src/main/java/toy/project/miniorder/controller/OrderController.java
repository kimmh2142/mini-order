package toy.project.miniorder.controller;

import lombok.RequiredArgsConstructor;
import org.springframework.web.bind.annotation.*;
import toy.project.miniorder.dto.AjaxResult;
import toy.project.miniorder.dto.OrderReqDTO;
import toy.project.miniorder.dto.OrderResDTO;
import toy.project.miniorder.service.OrderService;
...
public class OrderController {
    ...
    @PostMapping("/create")
    public AjaxResult insertOrder(@RequestBody OrderReqDTO orderReqDTO) {
        int result = orderService.insertOrder(orderReqDTO);
    ...
    @GetMapping("/list")
    public AjaxResult selectOrders(OrderReqDTO orderReqDTO) {
        List<OrderResDTO> result = orderService.selectOrders(orderReqDTO);
        Map<String, Object> data = new HashMap<>();
        data.put("result", result);
    ...
    @GetMapping("/{orderNo}")
    public AjaxResult selectOrder(@PathVariable("orderNo") int orderNo, OrderReqDTO orderReqDTO) {
        orderReqDTO.setOrderNo(orderNo);
        OrderResDTO result = orderService.selectOrder(orderReqDTO);
        Map<String, Object> data = new HashMap<>();
        data.put("result", result);
        AjaxResult ajaxResult;
        if (result != null) {
    ...
    @PutMapping("/{orderNo}")
    public AjaxResult updateOrder(@PathVariable("orderNo") int orderNo, @RequestBody OrderReqDTO orderReqDTO) {
        orderReqDTO.setOrderNo(orderNo);
        int result = orderService.updateOrder(orderReqDTO);
        Map<String, Object> data = new HashMap<>();
        data.put("result", result);
        AjaxResult ajaxResult;
        if (result > 0) {
    ...
    public AjaxResult deleteOrder(@PathVariable("orderNo") int orderNo, OrderReqDTO orderReqDTO) {
        orderReqDTO.setOrderNo(orderNo);
        int result = orderService.deleteOrder(orderReqDTO);
        Map<String, Object> data = new HashMap<>();
        data.put("result", result);
        AjaxResult ajaxResult;
        if (result > 0) {
